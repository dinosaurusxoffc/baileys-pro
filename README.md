# 🌱 @dinosaurus/baileys-pro

☕ 𝗙𝗼𝗿 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻: [ 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻 ](https://whatsapp.com/channel/0029VaqPkZ7ISTkQthJkuf1N)
...
>
>
> To be clear, I’m **not** the original maintainer of Baileys all respect goes to the amazing work behind [@whiskeysockets/baileys](https://github.com/WhiskeySockets/Baileys). I only created and maintained my own fork ([@dinosaurus/baileys-pro](https://www.npmjs.com/package/@dinosaurus/baileys-pro)) where I spent a **lot** of time improving and adapting things on my own.
>
...
# NPM
"dependencies": {
   "@dinosaurus/baileys-pro": "latest"
}

# GitHub
"dependencies": {
   "@dinosaurus/baileys-pro": "github:dinosaurusxoffc/baileys-pro"
}
```
...
```bash
# NPM
npm i @dinosaurus/baileys-pro@latest

# GitHub
npm i github:dinosaurusxoffc/baileys
```
...
```javascript
// --- ESM
import { makeWASocket } from '@dinosaurus/baileys-pro'

// --- CJS (tested and working on Node.js 24 ✅)
const { makeWASocket } = require('@dinosaurus/baileys-pro')
```

### 🌐 Connect to WhatsApp (Quick Step)

```javascript
import { makeWASocket, delay, DisconnectReason, useMultiFileAuthState } from '@dinosaurus/baileys-pro'
...
const connectToWhatsApp = async () => {
   ...
   dino.ev.on('connection.update', (update) => {
      ...
      if (connection === 'connecting' && !dino.authState.creds.registered) {
         await delay(1500)
         const code = await dino.requestPairingCode(myPhoneNumber)
   ...
   dino.ev.on('messages.upsert', async ({ messages }) => {
      for (const message of messages) {
         if (!message.message) continue

         console.log('🔔 Got new message', ':', message)
         await dino.sendMessage(message.key.remoteJid, {
            text: '👋🏻 Hello world'
         })
      }
   })
...
```javascript
import { makeWASocket, makeInMemoryStore, delay, DisconnectReason, useMultiFileAuthState } from '@dinosaurus/baileys-pro'
import { Boom } from '@hapi/boom'
import pino from 'pino'
...
const connectToWhatsApp = async () => {
   ...
   dino.ev.on('chats.upsert', () => {
      console.log('✉️ Got chats', store.chats.all())
   })

   dino.ev.on('contacts.upsert', () => {
...
const groupName = '@dinosaurus/baileys-pro'
...
dino.sendMessage(jid, {
   ...
   product: {
      ...
      signedUrl: 'https://www.npmjs.com/package/@dinosaurus/baileys-pro',
      title: '📦 Starseed (Premium)',
      url: 'https://www.npmjs.com/package/@dinosaurus/baileys-pro'
   },
...
dino.sendMessage('1211111111111@newsletter', {
   poll: {
      name: '🔥 Quiz',
      values: ['Yes', 'No'],
      correctAnswer: 'Yes',
...
```javascript
import { tokenizeCode } from '@dinosaurus/baileys-pro'
...
```javascript
dino.sendMessage([jidA, jidB, jidC], {
   text: 'Hello! 👋🏻'
})
```
...
dino.sendMessage(jid, {
   ...
   publisher: '🌟 Lia Wynn',
   description: '@dinosaurus/baileys-pro'
}, {
   quoted: message
})
```
...
dino.sendMessage(jid, {
   ...
   offerCode: '@dinosaurus/baileys-pro', // --- Optional
   offerUrl: 'https://www.npmjs.com/package/@dinosaurus/baileys-pro', // --- Optional
...
dino.sendMessage(jid, {
   ...
   cards: [{
      ...
      offerCode: '@dinosaurus/baileys-pro',
      offerUrl: 'https://www.npmjs.com/package/@dinosaurus/baileys-pro',
...
dino.sendMessage(jid, {
   ...
   externalAdReply: {
      ...
      url: 'https://www.npmjs.com/package/@dinosaurus/baileys-pro' // --- Optional, used for WhatsApp internal thumbnail caching and direct URL
   }
}, {
   quoted: message
})
...
const ids = await dino.findUserId(phoneNumber)
...
const ids = await dino.findUserId(lid)
...
await dino.requestPairingCode(phoneNumber, customPairingCode)
...
```javascript
import { getImageProcessingLibrary } from '@dinosaurus/baileys-pro'
...
```javascript
// --- Create a new one
dino.newsletterCreate('@dinosaurus/baileys-pro')

// --- Get info
dino.newsletterMetadata('1231111111111@newsletter')

// --- Demote admin
dino.newsletterDemote('1231111111111@newsletter', '6281111111111@s.whatsapp.net')

// --- Change owner
dino.newsletterChangeOwner('1231111111111@newsletter', '6281111111111@s.whatsapp.net')

// --- Change name
dino.newsletterUpdateName('1231111111111@newsletter', '📦 @dinosaurus/baileys-pro')

// --- Change description
dino.newsletterUpdateDescription('1231111111111@newsletter', '📣 Fresh updates weekly')

// --- Change photo
dino.newsletterUpdatePicture('1231111111111@newsletter', {
   url: 'path/to/image.jpg'
})

// --- Remove photo
dino.newsletterRemovePicture('1231111111111@newsletter')

// --- React to a message
dino.newsletterReactMessage('1231111111111@newsletter', '100', '💛')

// --- Get all subscribed newsletters
const newsletters = await dino.newsletterSubscribed()
...
dino.groupCreate('@dinosaurus/baileys-pro', ['628123456789@s.whatsapp.net'])

// --- Get info
dino.groupMetadata(jid)

// --- Get invite code
dino.groupInviteCode(jid)

// --- Revoke invite link
dino.groupRevokeInvite(jid)

// --- Leave group
dino.groupLeave(jid)

// --- Add participants
dino.groupParticipantsUpdate(jid, ['628123456789@s.whatsapp.net'], 'add')

// --- Remove participants
dino.groupParticipantsUpdate(jid, ['628123456789@s.whatsapp.net'], 'remove')

// --- Promote to admin
dino.groupParticipantsUpdate(jid, ['628123456789@s.whatsapp.net'], 'promote')

// --- Demote from admin
dino.groupParticipantsUpdate(jid, ['628123456789@s.whatsapp.net'], 'demote')

// --- Change name
dino.groupUpdateSubject(jid, '📦 @dinosaurus/baileys-pro')

// --- Change description
dino.groupUpdateDescription(jid, 'Updated description')

// --- Change photo
dino.updateProfilePicture(jid, {
   url: 'path/to/image.jpg'
})

// --- Remove photo
dino.removeProfilePicture(jid)

// --- Set group as admin only for chatting
dino.groupSettingUpdate(jid, 'announcement')

// --- Set group as open to all for chatting
dino.groupSettingUpdate(jid, 'not_announcement')

// --- Set admin only can edit group info
dino.groupSettingUpdate(jid, 'locked')

// --- Set all participants can edit group info
dino.groupSettingUpdate(jid, 'unlocked')

// --- Set admin only can add participants
dino.groupMemberAddMode(jid, 'admin_add')

// --- Set all participants can add participants
dino.groupMemberAddMode(jid, 'all_member_add')

// --- Enable or disable temporary messages with seconds format
dino.groupToggleEphemeral(jid, 86400)

// --- Disable temporary messages
dino.groupToggleEphemeral(jid, 0)

// --- Enable or disable membership approval mode
dino.groupJoinApprovalMode(jid, 'on')
dino.groupJoinApprovalMode(jid, 'off')
...
// --- Get pending invites
const invites = await dino.groupGetInviteInfo(code)

console.dir(invites, { depth: null })

// --- Accept group invite
dino.groupAcceptInvite(code)

// --- Get group info from link
const group = await dino.groupGetInviteInfo('https://chat.whatsapp.com/ABC123')
